...
const consolePage = await readFile(
  new URL("../src/pages/console.astro", import.meta.url),
  "utf8",
);
const loginPage = await readFile(
  new URL("../src/pages/console/login.astro", import.meta.url),
  "utf8",
);
...
const consoleLayout = await readFile(
  new URL("../src/layouts/ConsoleLayout.astro", import.meta.url),
  "utf8",
);
const consoleScript = (
  await Promise.all(
...
const sidebar = await readFile(
  new URL("../src/components/Sidebar.astro", import.meta.url),
  "utf8",
);
const publishApi = await readFile(
  new URL("../functions/api/publish.ts", import.meta.url),
  "utf8",
);
const musicUploadApi = await readFile(
  new URL("../functions/api/upload-music.ts", import.meta.url),
  "utf8",
);
const authApi = await readFile(
  new URL("../functions/api/auth.ts", import.meta.url),
  "utf8",
);
...
const consoleCardStyles = await readFile(
  new URL("../src/styles/console-cards.css", import.meta.url),
  "utf8",
);
...
test("console cards use the requested concise descriptions", () => {
  assert.match(consolePage, /<h2>登录<\/h2>\s*<p>登录到控制台<\/p>/);
  assert.match(consolePage, /<h2>上传文章<\/h2>\s*<p>上传Markdown文档<\/p>/);
  assert.match(consolePage, /<h2>上传音乐<\/h2>\s*<p>上传MP3音频文件<\/p>/);
  assert.match(consolePage, /<h2>上传歌词<\/h2>\s*<p>上传LRC歌词文件<\/p>/);
});
...
test("all console cards share the GitHub-style hover icon animation", () => {
  ...
  assert.match(consoleCardStyles, /translate\(12px, 12px\) scale\(0?\.86\)/);
});
...
test("music upload accepts every playlist exposed by the console", () => {
  assert.match(musicUploadApi, /getPlaylistDirectories/);
  assert.match(musicUploadApi, /playlistDirectories/);
});

test("file conflict dialog fills the content area with a frosted overlay", () => {
  assert.match(
    contentStyles,
    /dialog\[data-file-conflict\][\s\S]*?position:\s*fixed/,
  );
  assert.match(contentStyles, /inset:\s*0 0 0 299px/);
  assert.match(contentStyles, /backdrop-filter:\s*blur\(26px\)/);
...
test("file conflict actions provide symmetric overwrite and keep hover transforms", () => {
  assert.match(contentStyles, /data-overwrite\]:hover[^}]*rotateY\(\s*17deg\)/);
  assert.match(
    contentStyles,
    /data-keep-existing\]:hover[^}]*rotateY\(\s*-17deg\)/,
  );
  assert.match(contentStyles, /background:\s*#c94736/);
  assert.match(contentStyles, /background:\s*#3d9b68/);
});
