...
export class Presence {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (request.method === 'POST') {
      ...
      if (typeof input.steamOnline !== 'boolean' || !['offline', 'online', 'playing'].includes(input.neteaseState ?? '')) {
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (request.method === 'POST' && request.headers.get('authorization') !== `Bearer ${env.STATUS_REPORT_TOKEN}`) {
