interface Env { GITHUB_TOKEN: string; GITHUB_OWNER: string; GITHUB_REPO: string; GITHUB_BRANCH?: string; ADMIN_PASSWORD: string }
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  try {
    ...
    if (input.password !== env.ADMIN_PASSWORD) return json({ error: '管理员密码错误。' }, 401);
