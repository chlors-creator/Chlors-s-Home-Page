...
export function initializeAuth({ root, status }: AuthElements): () => void {
  ...
  const uploadCards = [...root.querySelectorAll<HTMLElement>('[data-upload-card]')];
  const loginCard = root.querySelector<HTMLElement>('[data-console-card="login"]');
  ...
  const authProbe = fetch('/api/auth', { credentials: 'same-origin' })
    ...
    .then((data) => {
      ...
      if (root.dataset.uploadPage && !authenticated) window.location.replace('/console/');
    })
    .catch(() => {
      authenticated = false;
    });
  ...
  uploadCards.forEach((card) => card.addEventListener('click', onUploadCardClick));
  ...
  return () => {
    active = false;
    uploadCards.forEach((card) => card.removeEventListener('click', onUploadCardClick));
