import { getPlaylistChildren } from "../../data/playlists";
import {
  readResponse,
  setConflictContent,
  toast,
...
const SUBPLAYLISTS = getPlaylistChildren("post-rock-punk").map(
...
function isConflict(data: Record<string, any>): data is ConflictResponse {
  return (
    data.error === "file_exists" && Boolean(data.incoming && data.existing)
  );
}
...
export function initializeSubplaylistSelector(root: HTMLElement): () => void {
  ...
  root
    .querySelectorAll<HTMLSelectElement>('select[name="directory"]')
    .forEach((select) => {
      ...
      const onChange = () => {
        field.hidden = select.value !== "post-rock-punk";
      };
...
export function initializeUploads({
  ...
  const musicForms = [
    ...root.querySelectorAll<HTMLFormElement>(
      "[data-mp3-form],[data-lrc-form]",
    ),
  ];
  const conflict = root.querySelector<HTMLDialogElement>(
    "[data-file-conflict]",
  );
  ...
  const musicSubmitHandlers = new Map<
    HTMLFormElement,
  ...
  const onOverwrite = async () => {
    ...
    const body =
      upload.kind === "article"
        ? JSON.stringify(Object.fromEntries(upload.formData))
        : upload.formData;
    const { response, data } = await request(
      upload.kind === "article" ? "/api/publish" : "/api/upload-music",
      body,
      upload.kind === "article",
    );
    conflict?.close();
  ...
  return () => {
    ...
    musicSubmitHandlers.forEach((handler, form) =>
